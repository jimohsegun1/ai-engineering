...
Same six steps as rag_pipeline.py, but the input document is a PDF
(data/sample.pdf) instead of a plain .txt file. Uses PyPDFLoader, which
requires the `pypdf` package.

A key difference from TextLoader: PyPDFLoader returns one Document per
PDF page (each with its own "page" number in metadata), rather than a
single Document for the whole file. So step 1 here already produces
multiple documents before step 2 (chunking) even runs.

Steps:
1. Prepare input document   <- PyPDFLoader, one Document per page
2. Chunking
3. Create embeddings
...
DOCUMENT_PATH = RAG_APP_DIR / "data" / "sample.pdf"
CHUNK_SIZE = 500
CHUNK_OVERLAP = 50
EMBEDDING_MODEL = "sentence-transformers/all-MiniLM-L6-v2"
PERSIST_DIRECTORY = RAG_APP_DIR / "db" / "pdf"
QOREBIT_BASE_URL = "https://api.qorebit.ai/v1"
CHAT_MODEL = "openai/gpt-4o"
TOP_K = 3

QUESTION = "What is HNSW and why is it useful for similarity search?"
...
# PyPDFLoader loads one Document per PDF page, each carrying its page
# number in metadata — unlike TextLoader, which returns a single
...
# --- Step 2: Chunking ---
# Split each page into overlapping chunks so retrieval can return
# focused passages instead of a whole page.
print_step(2, "Chunking")
splitter = RecursiveCharacterTextSplitter(chunk_size=CHUNK_SIZE, chunk_overlap=CHUNK_OVERLAP)
chunks = splitter.split_documents(documents)
print(f"Split into {len(chunks)} chunks")
for i, chunk in enumerate(chunks, start=1):
    label = f"Chunk {i}/{len(chunks)} (page {chunk.metadata['page']})"
    print_passage(label, chunk.page_content, len(chunk.page_content))
...
# and page metadata) on disk so they can be searched later without
# re-embedding. We wipe any previous run's data first so re-running
# this script doesn't keep appending duplicate chunks to the same
# collection.
...
for i, doc in enumerate(results, start=1):
    print_passage(f"Result {i}/{TOP_K} (page {doc.metadata['page']})", doc.page_content)
